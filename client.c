...
void sendPeriodic(int, unsigned int*, unsigned int*, int, int,
	struct sockaddr_in);
...
void sendPeriodic(int tick_size, unsigned int* periods,
	unsigned int* lens, int N, int sockfd, struct sockaddr_in serv_addr)
{
	struct timespec request = {(time_t)(tick_size / 1000),
		(long int)(tick_size % 1000) * 1000000L}, remn;
	char msg[MAX_LINE];
	int tick_n, i, n;
	...
	for (tick_n = 1; tick_n <= MAX_TICK; tick_n++)
	{
		...
		for (i = 0; i < N; i++)
		{
			if (tick_n % periods[i] == 0)
			{
				...
				// Send the message
				n = sendto(sockfd, (const char*)msg, strlen(msg),
						MSG_CONFIRM, (const struct sockaddr*) &serv_addr,
						sizeof(serv_addr));
				if (n < 0)
				{
					fprintf(stderr, "client: sendto (tick# %d, line# %d,\
 len = %d): ", tick_n, i, lens[i]);
					perror("");
					exit(EXIT_FAILURE);
				}
			}
 ...
 * */
void getCmdOptions(int argc, char *argv[], char **serv_IP, int *serv_port,\
	char **input_file, int *tick_size)
{
	*input_file = NULL;
	*tick_size = 100;
	int opt;
 ...
 * */
bool readSpecFile(char* file_path, unsigned int** periods,
	unsigned int** lens, int* N)
{
	FILE* fstream;
...
static char *rand_string(char *str, size_t len)
{
